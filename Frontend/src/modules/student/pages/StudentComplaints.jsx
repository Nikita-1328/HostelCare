...
const StudentComplaints = () => {
    ...
    const handleSubmit = async (section, subSection = null) => {
        ...
        if (category === 'Plumber') category = 'Plumber';
        if (category === 'Carpenter') category = 'Carpenter';
        ...
        try {
            ...
            if (response.ok) {
                alert(`Complaint for ${category}${subSection ? ' (' + subSection + ')' : ''} submitted successfully!`);
                handleInputChange(section, 'problem', '', subSection);
                handleInputChange(section, 'proof', null, subSection);
                fetchComplaints();
            } else {
                const errData = await response.json();
                alert(`Failed to submit complaint: ${errData.message}`);
            }
        } catch (error) {
                            ...
                            <tbody>
                                {pastComplaints.length === 0 ? (
                                    ...
                                    pastComplaints.map((past) => {
                                        let icon = 'fa-tools';
                                        let color = '#4e73df';
                                        if (past.category === 'Electrician') { icon = 'fa-bolt'; color = '#f6c23e'; }
                                        else if (past.category === 'Plumber') { icon = 'fa-faucet'; color = '#36b9cc'; }
                                        else if (past.category === 'Carpenter') { icon = 'fa-hammer'; color = '#e74a3b'; }
                                        else if (past.category === 'Cleaning') { icon = 'fa-broom'; color = '#1cc88a'; }
                                        ...
                                        if (past.status === 'Resolved') {
                                            badgeBg = '#e6fffa';
                                            badgeColor = '#1cc88a';
                                        } else if (past.status === 'In Progress') {
                                            badgeBg = '#eaecf4';
                                            badgeColor = '#4e73df';
                                        ...
                                        return (
                                            <tr key={past._id} style={{ borderBottom: '1px solid #f1f3f9' }}>
                                                <td style={{ padding: '15px', fontWeight: 700, color: '#4e73df', fontSize: '14px' }}>#{past._id.slice(-6).toUpperCase()}</td>
                                                <td style={{ padding: '15px' }}>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: color, fontWeight: 700, fontSize: '13px' }}>
                                                        <i className={`fas ${icon}`}></i> {past.category} {past.subCategory ? `(${past.subCategory})` : ''}
                                                    </div>
                                                </td>
                                                <td style={{ padding: '15px', color: '#5a5c69', fontSize: '14px' }}>{past.problem}</td>
                                                <td style={{ padding: '15px', color: '#858796', fontSize: '13px' }}>{new Date(past.createdAt).toLocaleDateString(undefined, {day: 'numeric', month: 'short', year: 'numeric'})}</td>
                                                <td style={{ padding: '15px' }}>
                                                    <span style={{ 
                                                        padding: '5px 12px', 
                                                        borderRadius: '20px', 
